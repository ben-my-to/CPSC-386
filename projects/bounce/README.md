...
* Use a hierarchy of objects to create a graphical demo using PyGame.
...
* `game/animation.py` example animation 
...
There are two steps to bouncing. The first step is determining that the ball has collided with something and the second step is to make the ball bounce.
...
The next step is to have the balls bounce. Consider the two circles in the figure above. _Circle A_ is touching _Circle B_ at a single point. Through that point is a tangent line. If we draw the radius from the center of _Circle A_ to the collision point and draw the radius of _Circle B_ from the collision point to it's center, we would see a line segment that is perpendicular to the tangent line. This line segment is the direction the ball must bounce around.
