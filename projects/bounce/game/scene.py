...
from game.ball import Ball, random_velocity
...
class Scene:
    ...
    def set_is_valid(self, value):
        """Sets the scene to be either valid/invalid"""
...
class BouncingBallsScene(Scene):
    ...
    def __init__(
        ...
        self._explosion_on = False
        self._pause_game = False
        self._balls = []

    def _draw_boundaries(self):
        """draws the yellow boundaries"""
        width = self._screen.get_size()[0]
        pygame.draw.rect(
            self._screen,
            rgbcolors.yellow,
            self._boundary_rect,
    ...
    def random_coord(self):
        ...
        offset = 100
        loc = self._boundary_rect.inflate(-offset, -offset)
        loc.move_ip(offset // 2, offset // 2)
        return uniform(loc.left, loc.width), uniform(loc.top, loc.height)

    def start_scene(self):
        """starts the bouncing ball scene"""
        super().start_scene()

        for index in range(self._num_balls):
            self._balls.append(
                Ball(index, *self.random_coord(), sound_on=False)
            )

            ball = self._balls[index]
            while any(
                ball.too_close(*other_ball.center, ball.radius * 3)
                for other_ball in self._balls[:index]
            ):
                self._balls[index].center.update(*self.random_coord())

            if not index:
                ball.set_velocity(*random_velocity(5, 5))
    ...
    def process_event(self, event):
        """listens and handles all events in the bouncing ball scene"""
        super().process_event(event)
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_e:
            self._explosion_on = not self._explosion_on
    ...
    def update_scene(self):
        """updates to bouncing ball scene"""
        if not self._pause_game:
            ...
            for ball in self._balls:
                if not self._boundary_rect.contains(ball.rect):
                    ball.circle.stay_in_bounds(*self._boundary_rect)
                    ball.wall_reflect(*self._boundary_rect)
            ...
            for index, ball in enumerate(self._balls):
                for other_ball in self._balls[index + 1 :]:
                    if ball.collide_with(other_ball):
                        ...
                        if ball.sound_on:
                            ball.bounce_sound_play()

                        if self._explosion_on:
            ...
            for ball in self._balls:
                if ball.name and ball.bounce_count < 1:
                    ball.halt()
